...
//This function copies values of one array into another. These arrays have to be 8 in size.
void copyArray(bool newData[], bool oldData[]);
...
int main()
{
    ...
    //If 8/8 values in the equivalence array are True, the two expressions are equivalent.
    if (eCounter == 8)
    {
...
//This function was ultimately created to keep from writing multiple for-loops that did the same thing. (DRY principle)
void copyArray(bool newData[], bool oldData[])
{
    for(int i = 0; i < 8 ; i++)
    {
...
void parseExpression(bool Expression[])
{
    ...
    bool var1[8] = {true, true, true, true, false, false, false, false}; //Represents P
    bool var2[8] = {true, true, false, false, true, true, false, false}; //Represents Q
    bool var3[8] = {true, false, true, false, true, false, true, false}; //Represents R
    ...
    while (character != 'E')
    {
        ...
        {
            //***Logic description For variables: The first variable always goes into Helper1, and the rest of the variables get put in Helper2,
            //***since only two variables are processed at the same time.
            case 'P':
                if (first == true)
                {
                    copyArray(Helper1, var1);
                    first = false;
                    tracker = 1; //Indicates Helper1 was modified last (for 'notting' values)
                }
                else
                {
                   copyArray(Helper2, var1);
                   tracker = 2;//Indicates Helper2 was modified last (for 'notting' values)
                }
                break;
            ...
            case 'Q':
                if (first == true)
                {  
                    copyArray(Helper1, var2);
                    first = false;  
                    tracker = 1; 
                }
                else
                {
                   copyArray(Helper2, var2);
                   tracker = 2;
                }
                break;
            ...
            case 'R':
                if (first == true)
                {
                    copyArray(Helper1, var3);
                    first = false;
                    tracker = 1;
                }
                else
                {
                   copyArray(Helper2, var3);
                   tracker = 2;
                }
            ...
            case 'A': //AND
                for (int i = 0 ; i< 8 ; i++)
                {
                    Helper1[i] = (Helper1[i] && Helper2[i]); //Example: the expression in parens is evaluated and a flag is assigned to a certain index in helper 1.
                }
                tracker  = 1; //Helper1 was modified
                break;
